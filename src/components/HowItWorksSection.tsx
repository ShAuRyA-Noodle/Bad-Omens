...
type PipelineStepData = (typeof pipelineSteps)[number];

// One instance per step. All hooks are called at this component's top level,
// satisfying the rules of hooks (the previous version called useTransform
// inside a .map callback, which is a hooks violation).
const PipelineStep = ({
  step,
  index,
  scrollYProgress,
  ...
  const range: [number, number, number] = [
    0,
    Math.min(1, index * 0.25),
    Math.min(1, (index + 0.5) * 0.25),
  ];
  const isActive = useTransform(scrollYProgress, range, [0, 0, 1]);
  const yOffset = useTransform(scrollYProgress, range, [50, 50, 0]);
  const borderColor = useTransform(isActive, (v) =>
    v > 0.5 ? "hsl(var(--primary))" : "rgba(255,255,255,0.2)"
  );
  const dotColor = useTransform(isActive, (v) =>
  ...
  return (
    <div className="relative z-10 grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-8">
      ...
      <motion.div
        ...
        style={{ borderColor }}
      >
        <motion.div className="w-[10px] h-[10px]" style={{ backgroundColor: dotColor }} />
      </motion.div>

      {/* Step ID / Execution Context */}
      <motion.div style={{ opacity: isActive, y: yOffset }} className="font-mono text-sm">
      ...
      <motion.div style={{ opacity: isActive, y: yOffset }} className="w-full">
